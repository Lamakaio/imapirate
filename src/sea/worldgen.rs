...
fn get_sprite_id(surroundings : [TileKind; 9], variant : u32) -> u32 {
    ...
    half_var + (1 + match surroundings {
        ...
        //sides
        [Forest, Sea(_), _, Forest, _, _, _, Forest, _] | [Forest, Forest, Sea(_), Forest, _, _, _, Forest, Sea(_)] => FOREST_SEA_N, //N
        [Forest, Forest, _, Sea(_), _, Forest, _, _, _] | [Forest, Forest, Sea(_), Forest, Sea(_), Forest, _, _, _] => FOREST_SEA_E, //E
        [Forest,  _, _, Forest, _, Sea(_), _, Forest, _] | [Forest, _, _, Forest, Sea(_), Forest, Sea(_), Forest, _] => FOREST_SEA_S, //S
        [Forest, Forest, _, _, _, Forest, _, Sea(_), _] | [Forest, Forest, _, _, _, Forest, Sea(_), Forest, Sea(_)] => FOREST_SEA_W, //W

        [Forest, Sand(_), _, Forest, _, _, _, Forest, _] | [Forest, Forest, Sand(_), Forest, _, _, _, Forest, Sand(_)] => FOREST_SAND_N, //N
        [Forest, Forest, _, Sand(_), _, Forest, _, _, _] | [Forest, Forest, Sand(_), Forest, Sand(_), Forest, _, _, _] => FOREST_SAND_E, //E
        [Forest,  _, _, Forest, _, Sand(_), _, Forest, _] | [Forest, _, _, Forest, Sand(_), Forest, Sand(_), Forest, _] => FOREST_SAND_S, //S
        [Forest, Forest, _, _, _, Forest, _, Sand(_), _] | [Forest, Forest, _, _, _, Forest, Sand(_), Forest, Sand(_)] => FOREST_SAND_W, //W

        [Sand(_), Sea(_), _, Sand(_), _, _, _, Sand(_), _] | [Sand(_), Sand(_), Sea(_), Sand(_), _, _, _, Sand(_), Sea(_)] => SAND_SEA_N, //N
        [Sand(_), Sand(_), _, Sea(_), _, Sand(_), _, _, _] | [Sand(_), Sand(_), Sea(_), Sand(_), Sea(_), Sand(_), _, _, _] => SAND_SEA_E, //E
        [Sand(_),  _, _, Sand(_), _, Sea(_), _, Sand(_), _] | [Sand(_), _, _, Sand(_), Sea(_), Sand(_), Sea(_), Sand(_), _] => SAND_SEA_S, //S
        [Sand(_), Sand(_), _, _, _, Sand(_), _, Sea(_), _] | [Sand(_), Sand(_), _, _, _, Sand(_), Sea(_), Sand(_), Sea(_)] => SAND_SEA_W, //W
        ...
        //inside
        [Forest, Forest, _, Forest, _, Forest, _, Forest, _] => FOREST - half_var as i32 + variant as i32,
        [Sand(_), x, _, y, _, z, _, w, _] 
        if x != Sea(false) && y != Sea(false) && z != Sea(false) && w != Sea(false) && 
        x != Sea(true) && y != Sea(true) && z != Sea(true) && w != Sea(true)
        => SAND,
...
pub fn generate_chunk(pos_x : i32, pos_y : i32, world_seed : usize) -> Vec<Vec<pirate_tilemap::Tile>> {
...
fn get_id_map(map : &[Vec<Tile>]) -> Vec<Vec<pirate_tilemap::Tile>> {
    let mut layer = Vec::new();
    for i in 0..(CHUNK_SIZE as usize) {
        layer.push(Vec::new());
        for j in 0..(CHUNK_SIZE as usize) {
            if i == 0 || j == 0 || i as i32 == CHUNK_SIZE - 1 || j as i32 == CHUNK_SIZE - 1 {
                layer[i].push(pirate_tilemap::Tile::Static((1 + SEA) as u32));
            }
            else {
                layer[i].push(pirate_tilemap::Tile::Static(get_sprite_id([
                    map[i][j].kind,
                    map[i-1][j].kind,
                    map[i-1][j+1].kind,
                    map[i][j+1].kind,
