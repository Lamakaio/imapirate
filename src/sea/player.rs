...
use pirate_tilemap::{Chunk, CollisionType, SCALING};
use super::worldgen::{TILE_SIZE, CHUNK_SIZE};
...
impl PlayerPositionUpdate {
    ...
    fn update(&mut self, t : &Vec4) {
        ...
        const TILE : i32 = TILE_SIZE * SCALING;
        let assumed_x = TILE * self.tile_x + TILE * CHUNK_SIZE * self.chunk_x - TILE * CHUNK_SIZE / 2;
        let assumed_y = TILE * self.tile_y + TILE * CHUNK_SIZE * self.chunk_y - TILE * CHUNK_SIZE / 2;
...
fn setup(
    ...
    //camera
        ...
        transform : Transform::from_translation(Vec3::new(0., 0., far - 0.1))
        .with_scale(ZOOM),
        ..Default::default()
        })
    //player
        ...
        SpriteSheetComponents {
            ...
            transform : Transform::from_translation(Vec3::new(0., 0., BOAT_LAYER))
            .with_scale(2.),
            ..Default::default()
        })
    ...
    for (mut player, mut player_translation) in &mut player_query.iter() {
        ...
        match pos_update.collision_status {
            ...
            CollisionType::Rigid => {
                ...
                if (stuck_forward.unwrap() && player.speed < 0.) || (!stuck_forward.unwrap() && player.speed > 0.){
                    *player_translation.x_mut() += c * player.speed * time.delta_seconds/3.;
                    *player_translation.y_mut() += s * player.speed * time.delta_seconds/3.;
                }
                else {
...
fn collision_system(
    ...
    if pos_update.changed_tile {
        let chunk = chunks.get(&(pos_update.chunk_x, pos_update.chunk_y)).unwrap();
        pos_update.collision_status = 
        *chunk.collision_map[0].get(pos_update.tile_y as usize).unwrap_or(&Vec::new())
                              .get(pos_update.tile_x as usize).unwrap_or(&CollisionType::None);
    }
}