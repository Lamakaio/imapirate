...
use crate::sea::{
    map::{ChunkGenEvent, ChunkGenEventReader, MapParam},
    tilemap::Chunk,
    tilemap::CollisionType,
    tilemap::Tile as MapTile,
    worldgen::CHUNK_SIZE,
};
...
fn island_generation_system(
    ...
    mut event_reader: Local<ChunkGenEventReader>,
    param: Res<MapParam>,
    events: Res<Events<ChunkGenEvent>>,
    ...
    for event in event_reader.reader.iter(&events) {
        if let Some(chunk) = chunks.get(&(event.x, event.y)) {
            let mut collisions = chunk.collision_map[0].clone();
        ...
        match receive_channel.receiver.try_recv() {
            ...
            Ok(mut data) => {
                if let Some(chunk) = chunks.get_mut(&(data.chunk_x, data.chunk_y)) {
                    chunk.collision_map[0] = data.collision_map;
